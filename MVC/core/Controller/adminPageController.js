        ...
        if(cookie != undefined) {
            ...
            if(connected === true && userType === 1) {
                ...
                for(let i = 0; i < info.recipeName.length; i++) {
                    let recipeUrl = '/recipe/' + info.recipeName[i].replace('%20', ' ').replace('%20', ' ').replace('%20', ' ') + '.html';
                    ...
                    cards = cards + '<img src="/' + info.recipePhoto[i] + `" alt="recipePhoto" class="imageCard">
                                                                            <div class="container">`;
                    cards = cards + '<h4><b id="recipe' + i + '" >' + info.recipeName[i].replace('%20', ' ').replace('%20', ' ') + '</b></h4>';
                ...
                for(let i = 0; i < infouser.usernames.length; i++) {
                    ...
                    `<div class="card" id="carduser` + i + `">
                            ...
                            <img src="/settingsButtonUser.png" alt="settings" class="settingsButton" onclick="changeLocation('/` + infouser.usernames[i].replace('%20', ' ').replace('%20', ' ').replace('%20', ' ')  + `.tfl')">
        ...
        if(cookie != undefined) {
            ...
            if(connected === true && userType === 1) {
                ...
                fs.readFile('core/View/userPage.html', async (err, buffer) => {
                    let username = request.url.substr(1, request.url.length-5).replace('%20', ' ').replace('%20', ' ');
    ...
    static async GET_RECIPES(request, response) {
        ...
        request.on('end', async () => { 
            ...
            for(let i = 0; i < answer.recipeName.length; i++) {
                let recipeUrl = '/recipe/' + answer.recipeName[i].replace('%20', ' ').replace('%20', ' ') + '.html';
                ...
                responseString = responseString + '<img src="/' + answer.recipePhoto[i] + `" alt="recipePhoto" class="imageCard">
                                                                        <div class="container">`;
                responseString = responseString + '<h4><b id="recipe' + i +  '" >' + answer.recipeName[i].replace('%20', ' ').replace('%20', ' ').replace('%20', ' ') + '</b></h4>';
        ...
        request.on('end', async () => { 
            ...
            for(let i = 0; i < answer.usernames.length; i++) {
                ...
                `<div class="card"  id="carduser` + i + `">
                        ...
                        <img src="/settingsButtonUser.png" alt="settings" class="settingsButton" onclick="changeLocation('/` + answer.usernames[i].replace('%20', ' ').replace('%20', ' ').replace('%20', ' ')  + `.tfl')">
        ...
        if(connected === true && userType === 1) {
            ...
            await request.on('end', async () => { 
                ...
                if(answerPhoto != null && answerPhoto != undefined)
                    answer1 = await userModel.modifyUserPhotoByName(newUsername,'data/users/' + newUsername.replace('%20', ' ').replace('%20', ' ').replace('%20', ' ') + "/user.jpg");
                if(answer1 === true && answer === true) {
                        response.statusCode = 202;
        ...
        if(cookie != undefined) {
            ...
            if(connected === true && userType === 1) {
                ...
                await request.on('end', async () => { 
                    ...
                    if(answer === true) {
                            response.statusCode = 202;
                            response.end('type:' + (type == 0 ? "user": 'admin'));
                            return ;
                    }
                    response.statusCode = 406;
                    response.end();
